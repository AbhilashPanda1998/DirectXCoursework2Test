...
void main(line GeometryShaderInput input[2], inout TriangleStream<PixelShaderInput> OutputStream)
{
    ...
    for (int i = 0; i < pointsOnCircle; i++)
    {
		...
		//Vertex 1
        output.posWorld = input[0].pos + float4(0.0, cos(theta) * 0.3, sin(theta) * 0.3, 0);
        output.norm = float4(0.0, cos(theta) * 0.3, sin(theta) * 0.3, 1);
        output.position = vPos + float4(0.0, cos(theta) * 0.3, sin(theta) * 0.3, 0);
        output.position = mul(output.position, projection);
        OutputStream.Append(output);

		//Vertex 2
        output.posWorld = input[1].pos + float4(0.0, cos(theta) * 0.3, sin(theta) * 0.3, 0);
        output.norm = float4(0.0, cos(theta) * 0.3, sin(theta) * 0.3, 1);
        output.position = vPos2 + float4(0.0, cos(theta) * 0.3, sin(theta) * 0.3, 0);
        output.position = mul(output.position, projection);
        OutputStream.Append(output);
    }
